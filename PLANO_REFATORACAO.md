...
### Fase H — Bug fixes capturados em snapshot ✅ (concluído 2026-05-05)

**Resultado:** 2 bugs pré-existentes catalogados durante B–G corrigidos em commits dedicados; suite 155 passed, **zero xfailed** (era 154 + 1 xfail).

#### H.1 `_clean_chapter_title_text` ✅ — `5d2fb5c`

Bug capturado nos goldens em Fase 0.1: patterns rodavam em ordem; `r'^([IVXLC]+...)'` casava antes dos específicos `CAPÍTULO`/`PARTE`, dropando a primeira letra C/I/V/X/L de títulos. Produção exibia `## APÍTULO 1 - INTRODUÇÃO` em vez de `## INTRODUÇÃO`.

**Fix duplo:** (1) reorder — específicos primeiro, IVXLC standalone por último; (2) lookahead `(?=[\s.\-–—]|$)` no IVXLC pra impedir match em "INTRODUÇÃO"/"VOLUME"/"CONCLUSÃO" (sem o lookahead, mesmo após reorder o pattern dropava o "I" inicial via match vazio dos qualificadores opcionais). Docstring marcada raw string pra silenciar `SyntaxWarning` do Python 3.14.
...
**Causa raiz:** classe `[^_]` permitia match spanear `](` no meio do link. Trace: `[^_]+(?:_[^_]+)*` greedy comia toda a string, backtracking matchava só a última ocorrência de `_pageN.yaml`.

**Fix:** classe restrita a filename-safe chars `[\w./-]+?` (lazy). Match não spaneia mais `]`, `(`, `)`, espaços.

Test renomeado de `..._known_buggy` → `test_apply_page_offset_yaml_refs_in_markdown_link`; xfail removido.

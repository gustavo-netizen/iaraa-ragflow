...
def main():
    ...
    parser.add_argument("--no-resume", action="store_true", help="禁用断点续传，重新分割所有PDF")
    ...
    if progress_manager and progress_manager.is_step_completed('split'):
        if existing_mapping:
            print(f"✅ 分割步骤已完成，跳过")
            print(f"   已分割: {existing_mapping.get('stats', {}).get('split_pdfs', 0)} 个PDF")
            print(f"   总块数: {existing_mapping.get('stats', {}).get('total_chunks', 0)}")
            return 0
